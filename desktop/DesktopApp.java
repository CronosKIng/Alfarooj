...
public class DesktopApp {
    ...
    private void changeLanguage(String langCode) {
        ...
        JOptionPane.showMessageDialog(frame, "Language changed to " + langCode + ". Please restart the application for full effect.", "Language", JOptionPane.INFORMATION_MESSAGE);
    }
    
    private void showAboutDialog() {
        JOptionPane.showMessageDialog(frame,
            "AL FAROOJ AL SHAMI Time Table System\nVersion 2.0\n\nDeveloped for AL FAROOJ AL SHAMI Restaurant\nSharjah, UAE\n\nFeatures:\n- Super Admin Dashboard\n- Admin Dashboard\n- User Dashboard\n- Location Validation\n- Attendance Tracking\n- User Management\n- Multi-department Support\n- Multi-language Support",
    ...
    private JPanel createUserPanel() {
        ...
        String deptDisplay = getDepartmentDisplay();
        JLabel userLabel = new JLabel("User: " + currentFullName + (deptDisplay.isEmpty() ? "" : " (" + deptDisplay + ")"));
        userLabel.setForeground(Color.WHITE);
        userInfoPanel.add(userLabel);
        ...
        JButton signInBtn = createStyledButton("SIGN IN", new Color(76, 175, 80));
        signInBtn.addActionListener(e -> recordAttendance("sign_in", "Sign In"));
        buttonsPanel.add(signInBtn);
        
        JButton signOutBtn = createStyledButton("SIGN OUT", new Color(244, 67, 54));
        signOutBtn.addActionListener(e -> recordAttendance("sign_out", "Sign Out"));
        buttonsPanel.add(signOutBtn);
        
        JButton historyBtn = createStyledButton("MY HISTORY", new Color(156, 39, 176));
    ...
    private String getDepartmentDisplay() {
        ...
        switch(currentDepartment) {
            ...
            default: return currentDepartment;
        }
    }
    ...
    private void deleteSelectedUser() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            ...
            SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
                ...
                protected Void doInBackground() {
                    try {
                        String response = sendDeleteRequest(API_URL + "delete_user/" + userId);
                        JsonObject jsonResponse = JsonParser.parseString(response).getAsJsonObject();
                ...
                protected void done() {
                    ...
                    if (success) {
                        ...
                        JOptionPane.showMessageDialog(frame, "Delete failed: " + message, "Error", JOptionPane.ERROR_MESSAGE);
                    }
                }
            };
            worker.execute();
    ...
    private void recordAttendance(String eventType, String eventName) {
        statusLabel.setText("Checking location...");
        
        SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
            private double latitude = 25.3065744;
            private double longitude = 55.4573264;
            ...
            protected Void doInBackground() {
                try {
                    String ipResponse = sendGetRequest("https://ipapi.co/json/");
                    JsonObject ipJson = JsonParser.parseString(ipResponse).getAsJsonObject();
                    if (ipJson.has("latitude") && ipJson.has("longitude")) {
                        latitude = ipJson.get("latitude").getAsDouble();
                        longitude = ipJson.get("longitude").getAsDouble();
                    }
                    
                    String json = String.format("{\"latitude\":%f,\"longitude\":%f}", latitude, longitude);
                    String response = sendPostRequest(API_URL + "validate_location", json);
                    JsonObject jsonResponse = JsonParser.parseString(response).getAsJsonObject();
                    locationValid = jsonResponse.get("within_location").getAsBoolean();
                    if (!locationValid) {
                        locationMsg = jsonResponse.get("message").getAsString();
                    }
                } catch (Exception e) {
                    locationMsg = e.getMessage();
                }
                return null;
            }
            ...
            protected void done() {
                if (!locationValid) {
                    statusLabel.setText("Location validation failed");
                    JOptionPane.showMessageDialog(frame, locationMsg, "Location Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }
                
                String location = String.format("Lat: %.6f, Lon: %.6f", latitude, longitude);
                ...
                SwingWorker<Void, Void> attendanceWorker = new SwingWorker<Void, Void>() {
                    private boolean success = false;
                    private String message = "";
                    ...
                    protected Void doInBackground() {
                        try {
                            ...
                            if (jsonResponse.has("timestamp")) {
                                timestamp = jsonResponse.get("timestamp").getAsString();
                            }
                        } catch (Exception e) {
                            message = e.getMessage();
                        }
                    ...
                    protected void done() {
                        ...
                        if (success) {
                            if (logArea != null) {
                                logArea.append("[" + time + "] " + eventName + " recorded at " + timestamp + " (UAE Time)\n");
                            }
                            JOptionPane.showMessageDialog(frame, message, "Success", JOptionPane.INFORMATION_MESSAGE);
                        } else {
                            if (logArea != null) {
            ...
            protected Void doInBackground() {
                try {
                    ...
                    if (jsonResponse.get("success").getAsBoolean()) {
                        ...
                        historyData = dataList.toArray(new Object[0][0]);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    private void loadUserData() {
        if (logArea != null) {
            logArea.setText("");
            String time = LocalDateTime.now(UAE_TIMEZONE).format(DateTimeFormatter.ofPattern("HH:mm:ss"));
            logArea.append("[" + time + "] Welcome to AL FAROOJ AL SHAMI Time Table System\n");
            logArea.append("[" + time + "] You are logged in as: " + currentFullName + " (" + getDepartmentDisplay() + ")\n");
        }
    }
